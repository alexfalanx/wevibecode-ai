...
✅ Be VERY SPECIFIC - not just "restaurant" but "bright modern restaurant interior natural sunlight elegant furniture"
✅ ALWAYS include brightness keywords: "bright", "sunlight", "daylight", "well-lit", "natural light"
✅ ALWAYS include quality keywords: "professional photography", "high quality", "vibrant colors"
✅ AVOID generic terms - be specific about setting, mood, lighting
✅ NEVER include brand names or trademarked terms
✅ Focus on atmosphere and lighting quality
...
async function fetchBrightImages(searchTerms: any, websiteType: string, vibe: string): Promise<any[]> {
  ...
  console.log('🖼️  Fetching BRIGHT images with enhanced terms:');
  ...
  console.log(`📊 Fetched ${images.length}/3 images, avg brightness: ${Math.round(images.reduce((sum, img) => sum + (img.rawBrightness || 0), 0) / images.length)}`);
  return images;
}
...
.hero-on-light .hero-content h1 {
  ...
  -webkit-text-fill-color: transparent;
  background-clip: text;
  filter: drop-shadow(0 3px 12px rgba(255, 255, 255, 0.9)) drop-shadow(0 1px 4px rgba(255, 255, 255, 0.7));
}
