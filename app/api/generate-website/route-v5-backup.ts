...
// COMPLETE v5.0 - PEXELS API FOR BETTER IMAGES ✅
// ✅ NEW: Switched from Unsplash to Pexels API (higher quality, better curation)
// Previous: Gallery (3 unique), Testimonials (avatars), Images (cuisine-specific), Menu (11 items), Logo (emoji-simple)
// Plus: Modern fonts, simple logos, gradients, all 13 sections working
...
async function fetchBrightImages(searchTerms: any, websiteType: string, vibe: string): Promise<any[]> {
  ...
  if (!PEXELS_API_KEY || PEXELS_API_KEY === 'demo') {
    console.error('❌ Pexels API key missing - Get free key at https://www.pexels.com/api/');
    return [];
  }
  ...
  for (let i = 0; i < searchQueries.length; i++) {
    ...
    try {
      ...
      if (data.photos && data.photos.length > 0) {
        ...
        console.warn(`⚠️  No images for: "${query}"`);
      }
    } catch (error: any) {
