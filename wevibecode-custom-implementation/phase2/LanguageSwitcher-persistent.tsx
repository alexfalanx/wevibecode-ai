// components/LanguageSwitcher.tsx (Phase 2 - Enhanced with persistence)
'use client';
...
export default function LanguageSwitcher() {
  ...
  const switchLanguage = async (newLocale: string) => {
    try {
      // 1. Save to cookie (for guests)
      Cookies.set('preferred_locale', newLocale, { expires: 365 });

      // 2. Save to database (for logged-in users)
      ...
      if (user) {
        ...
        if (error) {
          ...
          console.log('Language preference saved to profile');
        }
      }

      // 3. Update URL and navigate
